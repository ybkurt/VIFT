...
def eulerAnglesToRotationMatrixTorch(theta):
    '''
    Calculate the rotation matrix from Euler angles (roll, pitch, yaw)
    Input: theta - tensor of shape [..., 3]
    Output: R - tensor of shape [..., 3, 3]
    '''
    ...
    # Compute sin and cos of each angle
...
def mse_err_cal(pose_est, pose_gt):
    '''
    Calculate the mse of relative translation and rotation
    '''
    t_mse = np.mean(np.sum((pose_est[:, 3:] - pose_gt[:, 3:])**2, -1))
    r_mse = np.mean(np.sum((pose_est[:, :3] - pose_gt[:, :3])**2, -1))
    return t_mse, r_mse
